...
<script>
	$(function() {
		...
		*/
						 	...
						 	.on("click", function() {
								...
								if (confirm("정말 삭제할까요?")) {
					                ...
					                deptService.remove(deptno, function (result){
					                	if(result === 'SUCCESS') spanDelete.closest("tr").remove();
					                });
		                         } // if
							});
		...
		// 부서삭제
		// #tbl-dept > tbody > tr:nth-child(1) > td:nth-child(5) > span
		$("#tbl-dept > tbody > tr > td:nth-child(5) > span").on("click", function() {
			// alert("부서 삭제");
			if (confirm("정말 삭제할까요?")) {
				let deptno = $(this).data("deptno");
                deptService.remove(deptno, function (result){
                	if(result === 'SUCCESS') $(event.currentTarget).closest("tr").remove();
                });
			}
		});
